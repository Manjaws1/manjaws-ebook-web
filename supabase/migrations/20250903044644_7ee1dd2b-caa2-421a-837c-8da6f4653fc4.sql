-- Add indexes for better search performance (without CONCURRENTLY since it's not supported in transactions)
CREATE INDEX IF NOT EXISTS idx_ebooks_search 
...
CREATE INDEX IF NOT EXISTS idx_ebooks_status_downloads 
ON ebooks (status, downloads DESC) WHERE status = 'approved';

CREATE INDEX IF NOT EXISTS idx_ebooks_featured 
